...
exports.handler = function(event, context) {
    ...
    if (event && event.Records && event.Records.length >= 1 && event.Records[0].Sns && event.Records[0].Sns.Subject) {
        ...
        subject = (JSON.stringify(event.Records[0].Sns.Message));
        console.log('Subject not Available Check Message:', event.Records[0].Sns.Message);
        jsonObj = JSON.parse(event.Records[0].Sns.Message);
        if (jsonObj['detail-type'] && jsonObj['detail']) {
            subject = jsonObj['detail-type'].concat(' Status: ', jsonObj.detail.status || 'Unkown');
        }
        var tmpMessage = event.Records[0].Sns.Message.toLowerCase();
        if (tmpMessage.indexOf('visbodev') >= 0) {
            channel = '#aws_dev';
        } else if (tmpMessage.indexOf('visbostag') >= 0) {
            channel = '#aws_stag';
        } else if (tmpMessage.indexOf('visboprod') >= 0) {
            channel = 'aws_prod';
        } else {
            sendMessage = false;
        }
